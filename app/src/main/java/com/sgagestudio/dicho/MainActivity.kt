...
import com.sgagestudio.dicho.presentation.receipts.ReceiptJobDetailScreen
import com.sgagestudio.dicho.presentation.receipts.ReviewDetailScreen
import com.sgagestudio.dicho.presentation.receipts.ReviewScreen
...
private object ReceiptRoutes {
    ...
    const val QueueDetail = "receipt_job_detail"
    const val Review = "receipt_review"
    const val ReviewDetail = "receipt_review_detail"
    const val PreviewArg = "imageUri"
    const val JobIdArg = "jobId"
}
        ...
        NavHost(
            ...
            composable(ReceiptRoutes.Queue) {
                QueueScreen(
                    ...
                    onOpenReview = { navController.navigate(ReceiptRoutes.Review) },
                    onOpenDetail = { jobId ->
                        navController.navigate("${ReceiptRoutes.QueueDetail}/$jobId")
                    },
                )
            }
            composable(
                route = "${ReceiptRoutes.QueueDetail}/{${ReceiptRoutes.JobIdArg}}",
                arguments = listOf(navArgument(ReceiptRoutes.JobIdArg) { type = NavType.LongType }),
            ) { backStackEntry ->
                val jobId = backStackEntry.arguments?.getLong(ReceiptRoutes.JobIdArg) ?: 0L
                ReceiptJobDetailScreen(jobId = jobId, onBack = { navController.popBackStack() })
            }
            composable(ReceiptRoutes.Review) {
                ReviewScreen(
                    onBack = { navController.popBackStack() },
                    onOpenDetail = { jobId ->
                        navController.navigate("${ReceiptRoutes.ReviewDetail}/$jobId")
                    },
                )
            }
            composable(
                route = "${ReceiptRoutes.ReviewDetail}/{${ReceiptRoutes.JobIdArg}}",
                arguments = listOf(navArgument(ReceiptRoutes.JobIdArg) { type = NavType.LongType }),
            ) { backStackEntry ->
                val jobId = backStackEntry.arguments?.getLong(ReceiptRoutes.JobIdArg) ?: 0L
                ReviewDetailScreen(jobId = jobId, onBack = { navController.popBackStack() })
            }
        }
    }
}
